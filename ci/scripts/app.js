...
function renderSummary(summary) {
  ...
  if (totalBenchmarks) totalBenchmarks.textContent = summary.total_benchmarks ?? 0;
  ...
  if (improvementRate) improvementRate.textContent = `${summary.improvement_rate ?? 0}%`;
}
...
function renderTable(benchmarks) {
  ...
  benchmarks.forEach((benchmark) => {
    (benchmark.skills || []).forEach((skill) => {
      ...
      row.dataset.improvement = (normalizedSkill.improvement || '').toLowerCase();
      ...
      const improvementLabel = improvement === 'yes'
        ? 'Improvement'
        : improvement === 'no'
          ? 'Regression'
          : 'Neutral';
...
function filterTable() {
  ...
  rows.forEach((row) => {
    ...
    const matchImprovement = !improvement || row.dataset.improvement === improvement;

    row.style.display = matchProvider && matchModel && matchSkill && matchImprovement ? '' : 'none';
  });
}
...
function showDetails(index) {
  ...
  const beforeRating = judgment.option_a_rating || skill.baseline_rating || 'vague';
  ...
  const reasoning = skill.reasoning || judgment.reasoning || 'No reasoning provided';
  ...
  if (reasoningEl) reasoningEl.innerHTML = escapeHtml(reasoning).replace(/\n/g, '<br>');
  if (codeBeforeEl) codeBeforeEl.textContent = skill.before_code || '// No code generated';
